...
UserRouter.post('/login', async (req, res) => {
  try {
    ...
    const startSession = await Session.create({ tokenId, lastActivity: Date.now })
    
    res
      .status(201)
      .json({ msg: 'User logged in', token: token, user: existUser, session: startSession });
...
UserRouter.post('/register', async (req, res) => {
  try {
    ...
    if (name && user_name && email && password && account_type ) {
      ...
      const user = await User.create({ name, email, password: hashPassword, user_name, account_type });

      // Create Authentication token session.
      const tokenId = generateSessionToken();
      const startSession = await Session.create({ tokenId, lastActivity: Date.now })

      res.status(200).json({ msg: 'User registered successfully', user: user, session: startSession });
    } else {
